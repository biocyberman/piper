 ...
 */
object MakeSolidProjectHierarchy extends App {
  ...
  val parser = new OptionParser[Config]("MakeSolidProjectHierarchy$") {
    head("MakeSolidProjectHierarchy$", " - A utility program to convert a sthlm style project dir to a ua style one. \n" +
      "Run example: " +
      "./MakeSolidProjectHierarchy$ " +
      " --input_root <sthlm project root folder> " +
      " --out_root <root of uppsala style project>")

    opt[File]('i', "input_root") required () valueName ("A SOLiD style project directory.") action { (x, c) =>
  ...
  def parseSampleInfoFromFileHierarchy(
    ...
    val fastqFileRegexp =
      """^(.*)_(\w+(?:-\w+)?)_L(\d+)_R(\d)_(\d+)\.fastq\.gz$""".r
