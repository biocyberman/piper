...
/**
 * Utility program to convert generate AfMD meta data in a tsv file to UU SNP format.
 */
object AmfdGenerateXMLfromCSV extends App {

  val usage = "Missing input file. Usage is: java -classpath <classpath> molmed.apps.MakeAfMDInputDataHierarchy <meta file> <new root dir>"
  ...
  for (line <- lines) {
    ...
    // take all the last columns as input sequence files.
    val seqFiles = line.drop(4)


    // Sometime the meta data file contains more files than the folder...
//    if (seqFiles.forall( f => Files.exists(Paths.get(f)))) {
